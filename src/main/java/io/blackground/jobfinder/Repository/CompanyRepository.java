...
//	@Query(value = "SELECT id,name,website,about,city,location,industry,numbere FROM Company t", nativeQuery = true)
	@Query(value = "select i.name as industry,c.id id,c.website as website, c.about as about, c.city as city, c.location as location,c.numbere as numbere, c.name as name from industry i inner join company c on i.id=c.industry", nativeQuery = true)
